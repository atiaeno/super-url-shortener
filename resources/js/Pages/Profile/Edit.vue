                ...
                <div class="card-body">
                    <form @submit.prevent="submitProfile" class="form-stack">
                        <div class="field">
                            ...
                            <span v-if="profileForm.errors.name" class="field__error">{{ profileForm.errors.name }}</span>
                        </div>
                        <div class="field">
                            ...
                            <span v-if="profileForm.errors.email" class="field__error">{{ profileForm.errors.email }}</span>
                        </div>
                        <div v-if="mustVerifyEmail && user.email_verified_at === null" class="verify-notice">
                            <p>Your email address is unverified.</p>
                            <a :href="route('verification.send')" method="post" as="button">Resend verification email</a>
                            <span v-if="status === 'verification-link-sent'" class="success-msg">Verification link sent!</span>
                        </div>
                ...
                <div class="card-body">
                    <div class="photo-row">
                        ...
                        <div class="photo-actions">
                            <input ref="fileInput" type="file" accept="image/jpeg,image/png,image/gif" class="hidden" @change="onFileChange" />
                            <button type="button" @click="fileInput.click()" class="btn-secondary">Choose Photo</button>
                            <button v-if="avatarForm.avatar" type="button" @click="submitAvatar" :disabled="avatarForm.processing" class="btn-primary">
                    ...
                    <div class="providers-list">
                        <div v-for="p in providers" :key="p.key" class="provider-row">
                            ...
                            <div class="provider-actions">
                                ...
                                <a v-if="!isConnected(p.key)" :href="route('social.redirect', p.key)" class="btn-secondary btn-sm">Connect</a>
                                <button v-else @click="disconnect(p.key)" class="btn-ghost btn-sm">Disconnect</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <form @submit.prevent="updatePassword" class="form-stack">
                        <div class="field">
                            ...
                            <span v-if="passwordForm.errors.current_password" class="field__error">{{ passwordForm.errors.current_password }}</span>
                        </div>
                        <div class="field">
                            <label class="field__label">New Password</label>
                            <input v-model="passwordForm.password" type="password" autocomplete="new-password"
                                class="field__input" placeholder="••••••••" />
                            <span v-if="passwordForm.errors.password" class="field__error">{{ passwordForm.errors.password }}</span>
                        </div>
                        <div class="field">
                            ...
                            <input v-model="passwordForm.password_confirmation" type="password" autocomplete="new-password"
                                class="field__input" placeholder="••••••••" />
                            <span v-if="passwordForm.errors.password_confirmation" class="field__error">{{ passwordForm.errors.password_confirmation }}</span>
                        </div>
                        <div class="form-actions">
                            <button type="submit" class="btn-primary" :disabled="passwordForm.processing">
                                {{ passwordForm.processing ? 'Saving...' : 'Update Password' }}
                            </button>
                            <span v-if="passwordForm.recentlySuccessful" class="success-msg">Password updated!</span>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
                <div class="modal">
                    ...
                    <div class="field">
                        ...
                        <span v-if="deleteForm.errors.password" class="field__error">{{ deleteForm.errors.password }}</span>
                    </div>
